...
const calculateReferralRewardAmountInPaise = ({
    ...
    const cleanCommissionValue = Number(commissionValue || 0);
    const cleanPurchaseAmount = Number(purchaseAmountInPaise || 0);
    ...
    if (commissionType === "percentage") {
        return Math.max(
            0,
            Math.round((cleanPurchaseAmount * cleanCommissionValue) / 100)
        );
    }

    return Math.max(0, Math.round(cleanCommissionValue));
};

const buildReferralRewardResponse = (reward) => {
    if (!reward) {
        return null;
    }
...
const createPendingReferralRewardForPaidPurchase = async ({ purchase }) => {
    ...
    try {
        ...
        const rewardAmountInPaise = calculateReferralRewardAmountInPaise({
            commissionType: partner.commissionType,
            commissionValue: partner.commissionValue,
            purchaseAmountInPaise: purchase.amountInPaise,
        });
